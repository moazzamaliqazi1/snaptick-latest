...
import { Box, Paper, Grid, Typography, Button, Avatar, Chip } from "@mui/material";
...
const DoctorAppointments = (props) => {
  ...
  const getAppointment = async () => {
    try {
      const response = await user.listAppointment("doctor_id", token);
                  ...
                  return (
                    <Grid item xs={12} md={4} key={item}>
                      <Paper elevation={5} sx={{ p: 1 }}>
                        ...
                        >
                          <Box sx={{ mr: 1 }}>
                            <Avatar alt={item.user_details.user_name} src={item.user_details.profile_image ? item.user_details.profile_image: AvatarImage} />
                          </Box>
                          <Box
                            sx={{
                              mr: 1,
                        ...
                        <Grid container spacing={2}>
                          ...
                          <Grid item xs={6}>
                            <Chip
                              label={item.is_checked ? "Completed" : "Not Checked"}
                          ...
                          <Grid item xs={6}>
                            <Chip
                              label={moment(item.start_time).format("h:mm:ss a")}
                            />
                          </Grid>
                          <Grid item xs={6}>
                            Day
                          ...
                          <Grid item xs={6}>
                            <Chip
                              color="error"
                              label={item.payment_verified ? "Verified": 'Not Verified'}
                              sx={{ color: "white" }}
                            />
                          </Grid>
                          <Grid item xs={6}>
                            ...
                            <Chip
                              ...
                              onClick={() => {
                                if(item.meeting_link){
                                  navigate(`/meeting/${item.meeting_link}/${item.token}/${profile.user_name}?appointment_id=${item.id}`)
                                }
                              }}
                            />
                          </Grid>
